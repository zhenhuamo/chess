...
export const getEvaluateGameParams = (game: Chess): EvaluateGameParams => {
  // chess.js verbose history does not include before/after FEN by default.
  // Reconstruct FENs by replaying moves from the game starting position (or FEN header).
  const verbose = game.history({ verbose: true }) as Array<{
  ...
  const sim = new Chess();
  if (startFen) {
    try { sim.load(startFen); } catch { /* ignore invalid header */ }
  }
  ...
  for (const mv of verbose) {
    ...
    uciMoves.push(uci);
    // Apply move on simulator
    try { sim.move({ from: mv.from as any, to: mv.to as any, promotion: mv.promotion as any }); } catch { /* ignore */ }
  }
  // FEN after the last move (final position)
  fens.push(sim.fen());
