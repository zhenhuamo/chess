...
const fetchLichessEval = async (
  ...
  try {
    ...
    // Network timeout is expected in cloud-first fallback (we then use local engine).
    // Avoid noisy console errors for AbortSignal timeouts or fetch aborts.
    // Only log unexpected errors in development if needed.
    // if (process.env.NODE_ENV !== 'production') {
    //   console.debug('[cloud-eval] fallback:', (error as any)?.message || error);
    // }
