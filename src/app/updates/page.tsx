...
export default function UpdatesPage() {
  ...
  const [filter, setFilter] = (typeof window !== 'undefined')
    ? (window as any).React?.useState?.('') || [ '', ()=>{} ]
    : ['',''] as any;
  const filterText: string = Array.isArray(filter)? filter[0] : '';
  const setFilterText: (v: string)=>void = Array.isArray(filter)? filter[1] : (()=>{});

  const filt = (v: string) => v.toLowerCase().includes((filterText||'').toLowerCase());
  const list = filterText ? updates.filter(u => filt(u.title) || filt(u.date) || (u.items||[]).some(it => filt(it))) : updates;
      ...
      <Paper variant="outlined" sx={{ p: { xs: 2, md: 3 }, width: '100%', maxWidth: 900 }}>
        <Stack spacing={2}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <TextField size="small" placeholder="搜索（标题/内容/日期）" fullWidth value={filterText} onChange={(e)=> setFilterText(e.target.value)} />
