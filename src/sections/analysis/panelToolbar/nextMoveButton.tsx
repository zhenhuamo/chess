...
export default function NextMoveButton() {
  ...
  const addNextGameMoveToBoard = useCallback(() => {
    ...
    if (nextMove) {
      playBoardMove({ from: nextMove.from, to: nextMove.to, promotion: nextMove.promotion, comment });
    }
  }, [isButtonEnabled, boardHistory, game, playBoardMove]);

  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === "ArrowRight") addNextGameMoveToBoard();
    };
  ...
  return (
    <Tooltip title="Go to next move">
      <Grid>
        <IconButton onClick={() => addNextGameMoveToBoard()} disabled={!isButtonEnabled} sx={{ paddingX: 1.2, paddingY: 0.5 }}>
